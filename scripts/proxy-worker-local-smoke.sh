...
UPSTREAM_LOG="${TMPDIR:-/tmp}/airlock-proxy-worker-upstream.$$"
PROXY_LOG="${TMPDIR:-/tmp}/airlock-proxy-worker.$$"
UPSTREAM_PID=""
PROXY_PID=""
...
grep -q "Authorization: Bearer proxy-worker-local-token" "$UPSTREAM_LOG"
...
if grep -q "proxy-worker-local-token" "$PROXY_LOG"; then
  echo "proxy-worker logs leaked the local secret" >&2
  exit 1
fi

echo "proxy-worker local smoke passed"

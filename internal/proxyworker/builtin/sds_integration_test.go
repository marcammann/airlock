...
import (
	...
	workersds "github.com/marcammann/airlock/internal/proxyworker/sds"
...
func TestSDSServerFetchSecretsReturnsLeafForResourceName(t *testing.T) {
	...
	server := workersds.NewServer(ca, log)
	...
	if response.GetTypeUrl() != workersds.SecretTypeURL {
		t.Fatalf("type_url = %q, want %q", response.GetTypeUrl(), workersds.SecretTypeURL)
	}
...
func TestSDSServerRefreshesLeafAfterCacheWindow(t *testing.T) {
	...
	server := workersds.NewServer(ca, workertel.NewMemoryEventLog())
...
func TestSDSServerVersionInfoTracksLeafCertificateContent(t *testing.T) {
	certPEM, keyPEM, err := GenerateCertificateAuthority("airlock stable sds version ca")
	if err != nil {
		t.Fatal(err)
	}
	ca, err := parseCertificateAuthorityWithCacheWindow(certPEM, keyPEM, 2*time.Second)
	if err != nil {
		t.Fatal(err)
	}
	server := workersds.NewServer(ca, workertel.NewMemoryEventLog())
	...
	first, err := server.FetchSecrets(context.Background(), request)
	if err != nil {
		t.Fatal(err)
	}
	second, err := server.FetchSecrets(context.Background(), request)
	if err != nil {
		t.Fatal(err)
	}
	if first.GetVersionInfo() == "" {
		t.Fatal("VersionInfo is empty")
	}
	if first.GetVersionInfo() != second.GetVersionInfo() {
		t.Fatalf("VersionInfo changed while leaf was cached: first=%s second=%s", first.GetVersionInfo(), second.GetVersionInfo())
	}
	...
	rotated, err := server.FetchSecrets(context.Background(), request)
	if err != nil {
		t.Fatal(err)
	}
	if rotated.GetVersionInfo() == first.GetVersionInfo() {
		t.Fatalf("VersionInfo = %s after leaf rotation, want changed version", rotated.GetVersionInfo())
	}
}

func fetchSDSLeafCertificate(t *testing.T, server *workersds.Server, request *discoveryv3.DiscoveryRequest) *x509.Certificate {
	t.Helper()
